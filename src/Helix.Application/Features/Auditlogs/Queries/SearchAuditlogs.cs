...
public sealed class SearchAuditlogs(IDbContext context, ILoggedInUser loggedInUser) : IHandler
{
    ...
    public async Task<Result<List<Auditlog>>> Handle(Request request, CancellationToken cancellationToken = default)
    {
        ...
        {
            // Upper-cased on both sides, as SearchDrives does: SQLite's instr() is
            // case-sensitive, and letters are always stored uppercase.
            string term = request.SearchTerm.ToUpperInvariant();
            ...
            auditlogsQuery = auditlogsQuery.Where(a =>
                (a.EntityName != null && a.EntityName.ToUpper().Contains(term)) ||
                (a.EntityLetter != null && a.EntityLetter.Contains(term)) ||
                (a.Detail != null && a.Detail.ToUpper().Contains(term)) ||
                (a.Message != null && a.Message.ToUpper().Contains(term)));
        }
